...
extern std::map<int, Vehicle *> otherVehicles;
...
void HUD::Draw()
{
	...
	if(vehicle) {
		glColor3f(1, 0, 0);
		DrawGauge(200+winWidthOff, 280, 210, 0, 10, abs(vehicle->getSpeed()), "Speed");
		glColor3f(1, 1, 0);
		DrawGauge(600+winWidthOff, 280, 210, -15, 15, vehicle->getSteering(), "Steer");
	}

	if(! RemoteDataManager::IsConnected()) {
		glColor3f(0.5f, .5f, 0.5f);
		RenderString("No Connection", 330 + winWidthOff, 50, GLUT_BITMAP_TIMES_ROMAN_24);
	}
	else {
		glColor3f(0, 1, 0);
		char buff[80];
		sprintf(buff, "Connected: %d", (otherVehicles.size() + 1));
		RenderString(buff, 330 + winWidthOff, 50, GLUT_BITMAP_TIMES_ROMAN_24);
	}
